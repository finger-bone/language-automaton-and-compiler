...
## Correspondence between DFA and Regular Grammar
...
If it is the second case, we create a state $A$ and an special accept state $qa$ if there doesn't exist one. Then adding an edge with label $a$ from $A$ to $qa$.

Obviously, by constructing the automa, we can test all strings in the given regular language.
