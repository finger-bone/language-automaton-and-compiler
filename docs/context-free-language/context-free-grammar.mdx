...
That's equal to say, if a sentence has two different parse trees, then the grammar is ambiguous.
...
If you were to write the their parse tree, you can see that they are different.
...
For unambiguous language, there is always a way that you can scan the sentence from left to right one by one, and find the only possible derivation (if there exists two, it's ambiguous). This is called an $LL(1)$ grammar. The first $L$ stands for left starting. The second $L$ stands for leftmost derivation. $1$ means that the grammar only need to look ahead of one symbol to know which rule to apply. 

$LL(1)$ is the level that most programming languages are at.
